...
class GraphLoader {
 ...
 public:
  ...
  virtual void loadLP(const std::string &fileName) {
    ...
    if (!indata.is_open()) {
     //ROS_ERROR( "[GraphLoader] Parameter file could not be opened" );
    }
  ...
  size_t getLPSize(){
    ...
    if (!status.ok()) {
       //MELO_INFO_STREAM("[GraphLoader] LP: " << status.ToString());
    }else{

    //MELO_INFO_STREAM("[GraphLoader] LP: " << netShape[0].scalar<int>()());
    }
  ...
  size_t getParamsSize(const std::string &scope) {
    ...
    if (!status.ok()) {
       //MELO_INFO_STREAM( status.ToString());
      std::cout << status.ToString() << std::endl;
    }else{

    //MELO_INFO_STREAM("[GraphLoader] N params: " << paramV[0].scalar<int>()());
    }
